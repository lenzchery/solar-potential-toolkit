...
Global Solar Atlas exports every country's GHI raster under the same generic, non-country-specific filename - downloading all six directly into this folder as-is would overwrite each previous file, since they would all share one identical name. Each downloaded file must therefore be **renamed** to the country-specific filename listed below immediately after download, before placing it in this folder.

| Country | Download page | Required filename (after renaming) |
|---|---|---|
...
This renamed filename must match `raster_filename` in the corresponding `config/<country>.json` - the pipeline reads this field directly and does not infer the filename automatically.
